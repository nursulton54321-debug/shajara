...
if (isset($update['message'])) {
    ...
    if ($step == 'admin_input_warn' && $chat_id == ADMIN_TG_ID) {
        ...
        sendTelegram('sendMessage', [
            'chat_id' => $target,
            'text' => "⚠️ <b>ADMIN XABARI:</b>\n\n$text",
            'parse_mode' => 'HTML'
        ]);
        sendTelegram('sendMessage', [
            'chat_id' => $chat_id,
            'text' => "✅ Xabar ID: $target ga yuborildi.",
            'reply_markup' => btn_main_menu($chat_id)
        ]);
        setStep($chat_id, 'none', []);
        exit;
    ...
    if (strpos($text, '/pin ') === 0 && $chat_id == ADMIN_TG_ID) {
        ...
        sendTelegram('sendMessage', [
            'chat_id' => $chat_id,
            'text' => "✅ Sayt himoya PIN-kodi muvaffaqiyatli o'zgartirildi!\n\n🔑 **Yangi PIN-kod:** `$new_pin`",
            'parse_mode' => 'Markdown'
        ]);
        exit;
    }
    ...
    if (($text == '/backup' || strpos($text, 'Zaxira') !== false || strpos($text, 'Backup') !== false) && $chat_id == ADMIN_TG_ID) {
        sendTelegram('sendMessage', [
            'chat_id' => $chat_id,
            'text' => "⏳ Shajara nusxalanmoqda (Zip va SQL)..."
        ]);
    ...
    if (strpos($text, '/m_') === 0) {
        ...
        if ($s_data && ($chat_id == ADMIN_TG_ID || $s_data['added_by_tg_id'] == $chat_id)) {
            ...
            sendTelegram('sendMessage', [
                'chat_id' => $chat_id,
                'text' => "❌ Sizda bu shaxsni boshqarish huquqi yo'q."
            ]);
        }
        exit;
    }
    ...
    if ($step == 'none') {
        ...
        if (strpos($text, 'Foydalanuvchilar (Admin)') !== false && $chat_id == ADMIN_TG_ID) {
            ...
            sendTelegram('sendMessage', [
                'chat_id' => $chat_id,
                'text' => "👥 <b>Bot foydalanuvchilari ro'yxati:</b>",
                'parse_mode' => 'HTML'
            ]);
        ...
        if (strpos($text, 'Barcha shaxs') !== false) {
            ...
            while ($r = $res->fetch_assoc()) {
                $barcha[$r['id']] = $r;
                if ($r['ota_id']) {
                    $children[$r['ota_id']][] = $r['id'];
                } elseif ($r['ona_id']) {
                    $children[$r['ona_id']][] = $r['id'];
                }
            }
            ...
            $buildTreeText = function ($id, $level) use (&$buildTreeText, &$barcha, &$children, &$visited, $chat_id) {
                ...
                if ($chat_id == ADMIN_TG_ID || $p['added_by_tg_id'] == $chat_id) {
                    $t .= " 👉 /m_{$p['id']}";
                }
                $t .= "\n";
                ...
                if ($spouse_id && isset($barcha[$spouse_id]) && !isset($visited[$spouse_id])) {
                    ...
                    if ($chat_id == ADMIN_TG_ID || $sp['added_by_tg_id'] == $chat_id) {
                        $t .= " 👉 /m_{$sp['id']}";
                    }
                    $t .= "\n";
                    if (isset($children[$spouse_id])) {
                        foreach ($children[$spouse_id] as $ch_id) {
                            $t .= $buildTreeText($ch_id, $level + 1);
                        }
                    }
                }

                if (isset($children[$id])) {
                    foreach ($children[$id] as $ch_id) {
                        $t .= $buildTreeText($ch_id, $level + 1);
                    }
                }
                return $t;
            ...
            foreach ($roots as $rid) {
                $final_text .= $buildTreeText($rid, 0);
            }
            foreach ($barcha as $id => $p) {
                if (!isset($visited[$id])) $final_text .= $buildTreeText($id, 0);
            }

            $lines = explode("\n", $final_text);
            $msg = "";
            foreach ($lines as $line) {
                if (mb_strlen($msg . $line) > 3800) {
                    sendTelegram('sendMessage', [
                        'chat_id' => $chat_id,
                        'text' => $msg,
                        'parse_mode' => 'HTML'
        ...
        if (strpos($text, 'Statistika') !== false) {
            ...
            if ($jami > 0) {
                ...
                while ($r = $res_av->fetch_assoc()) {
                    $barcha_ota[$r['id']] = $r['ota_id'];
                }
                $max_depth = 1;
                foreach ($barcha_ota as $id => $ota_id) {
                    $depth = 1;
                    $curr = $id;
                    while (!empty($barcha_ota[$curr])) {
                        $curr = $barcha_ota[$curr];
                        $depth++;
                        if ($depth > 20) break;
                    }
                    if ($depth > $max_depth) $max_depth = $depth;
                }
                $e_foiz = round(($erkak / $jami) * 100);
                $a_foiz = round(($ayol / $jami) * 100);
        ...
        if (strpos($text, "Qidiruv") !== false) {
            ...
            sendTelegram('sendMessage', [
                'chat_id' => $chat_id,
                'text' => "🔍 <b>Qidirmoqchi bo'lgan shaxsning ismini kiriting:</b>\n<i>(Ismidan yoki familiyasidan 3 ta harf yozib yuboring)</i>",
                'parse_mode' => 'HTML',
                'reply_markup' => btn_cancel()
            ]);
            exit;
        ...
        if (strpos($text, "Mening ariza") !== false) {
            ...
            if ($res->num_rows == 0) {
                sendTelegram('sendMessage', [
                    'chat_id' => $chat_id,
                    'text' => "Sizda kutilayotgan arizalar yo'q."
                ]);
            }
            while ($a = $res->fetch_assoc()) {
                ...
                sendTelegram('sendMessage', [
                    'chat_id' => $chat_id,
                    'text' => "⏳ Kutilmoqda: {$a['ism']} {$a['familiya']}",
                    'reply_markup' => $kb
                ]);
            }
            exit;
    ...
    if (strpos($text, "Yangi shaxs") !== false) {
        ...
        $temp['ism'] = $text;
        setStep($chat_id, 'add_familiya', $temp);
        sendTelegram('sendMessage', [
            'chat_id' => $chat_id,
            'text' => "✍️ <b>2. Familiyani kiriting:</b>\n<i>(Masalan: Matayev)</i>",
            'parse_mode' => 'HTML'
        ]);
    } elseif ($step == 'add_familiya') {
        $temp['familiya'] = $text;
        setStep($chat_id, 'add_ota_ismi', $temp);
        sendTelegram('sendMessage', [
            'chat_id' => $chat_id,
            'text' => "✍️ <b>3. Otasining ismi (Sharifi):</b>\n<i>(Masalan: Elyorovich)</i>",
            'parse_mode' => 'HTML',
            'reply_markup' => btn_skip_cancel()
        ]);
    } elseif ($step == 'add_ota_ismi') {
        $temp['otasining_ismi'] = (strpos($text, 'tkazib') !== false) ? "" : $text;
        setStep($chat_id, 'add_jins', $temp);
        sendTelegram('sendMessage', [
            'chat_id' => $chat_id,
            'text' => "🚻 <b>4. Jinsini tanlang:</b>",
            'parse_mode' => 'HTML',
            'reply_markup' => btn_gender()
        ]);
    } elseif ($step == 'add_jins') {
        $temp['jins'] = (strpos($text, 'Erkak') !== false) ? 'erkak' : 'ayol';
        setStep($chat_id, 'add_sana', $temp);
        sendTelegram('sendMessage', [
            'chat_id' => $chat_id,
            'text' => "📅 <b>5. Tug'ilgan sana (kk.oo.yyyy):</b>\n<i>(Masalan: 01.03.2013)</i>",
            'parse_mode' => 'HTML',
            'reply_markup' => btn_cancel()
        ]);
        ...
        if (preg_match('/^(0[1-9]|[12][0-9]|3[01])\.(0[1-9]|1[012])\.(19|20)\d\d$/', $text)) {
            ...
            if (db_query("SELECT id FROM shaxslar WHERE ism='$i' AND familiya='$f' AND tugilgan_sana='$db_sana'")->num_rows > 0) {
                ...
                $temp['sana'] = $db_sana;
                setStep($chat_id, 'add_tel', $temp);
                sendTelegram('sendMessage', [
                    'chat_id' => $chat_id,
                    'text' => "📞 <b>6. Telefon raqami:</b>\n<i>(Masalan: +998901234567)</i>",
                    'parse_mode' => 'HTML',
                    'reply_markup' => btn_skip_cancel()
                ]);
            }
            ...
            sendTelegram('sendMessage', [
                'chat_id' => $chat_id,
                'text' => "❌ Xato format! Namunadagidek yozing: 01.03.2013"
            ]);
        }
    } elseif ($step == 'add_tel') {
        $temp['telefon'] = (strpos($text, 'tkazib') !== false) ? "" : $text;
        setStep($chat_id, 'add_kasb', $temp);
        sendTelegram('sendMessage', [
            'chat_id' => $chat_id,
            'text' => "💼 <b>7. Kasbi yoki Mutaxassisligi:</b>\n<i>(Masalan: Nafaqada, o'qituvchi, talaba, uy bekasi va h.k.)</i>",
            'parse_mode' => 'HTML',
            'reply_markup' => btn_skip_cancel()
        ]);
    } elseif ($step == 'add_kasb') {
        $temp['kasb'] = (strpos($text, 'tkazib') !== false) ? "" : $text;
        setStep($chat_id, 'add_foto', $temp);
        sendTelegram('sendMessage', [
            'chat_id' => $chat_id,
            'text' => "📸 <b>8. Rasmini yuboring:</b>",
            'parse_mode' => 'HTML',
            'reply_markup' => btn_skip_cancel()
        ]);
    } elseif ($step == 'add_foto') {
        $temp['foto'] = isset($update['message']['photo']) ? end($update['message']['photo'])['file_id'] : "";
        setStep($chat_id, 'ask_ota', $temp);
        sendTelegram('sendMessage', [
            'chat_id' => $chat_id,
            'text' => "👨‍👦 <b>9. Otasining ismi qanday?</b>\n<i>(Baza ichidan qidirish uchun 3 ta harf yozib yuboring)</i>",
            'parse_mode' => 'HTML',
            'reply_markup' => btn_skip_cancel()
        ]);
    }
